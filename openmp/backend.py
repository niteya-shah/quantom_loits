...
def load_extension(verbose=False):
    ...
    _EXTENSION = load(
        name="quantom_loits_openmp",
        sources=[str(root / "bindings.cpp"), str(root / "loits_core.cpp")],
        build_directory=str(build),
        extra_cflags=["-O3", "-march=native", "-DNDEBUG", "-fno-math-errno", "-fopenmp"],
...
class _OpenMPLOITSFunction(torch.autograd.Function):
    @staticmethod
...
class OpenMPLOITS(nn.Module):
    ...
    def forward(self, theory_outputs, n_events):
        ...
        with _region(self.profile_regions, "loits::autograd::forward"):
            return _OpenMPLOITSFunction.apply(
                x_bins,
                xsec_x,
                q2_bins,
                xsec_q2,
