...
from openmp.backend import load_extension
...
def test_openmp_forward_and_reverse_vjp_match_torch_for_same_samples():
    theory = make_theory()
    params = make_params()
    ...
    grad_omp_x, grad_omp_q = load_extension().backward(
        upstream,
        x_bins,
        xsec_x,
        q2_bins,
    ...
    torch.testing.assert_close(grad_omp_x, grad_ref_x, rtol=1e-10, atol=2e-12)
    torch.testing.assert_close(grad_omp_q, grad_ref_q, rtol=1e-10, atol=2e-12)


def test_openmp_public_autograd_path():
    torch.manual_seed(17)
    theory = make_theory()
    params = make_params()
    sampler = LOITS(backend="openmp", device="cpu")
...
def test_openmp_gan_training_and_region_profiler():
    trainer = GANTrainer(
        backend="openmp",
        device="cpu",
        n_events=30,
    ...
    assert "loits::forward::cdf_x" in names
...
def test_openmp_gan_training_is_reproducible():
    first = GANTrainer(
        backend="openmp",
        device="cpu",
        n_events=30,
