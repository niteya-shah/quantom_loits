...
# Final launch geometry. DPC++ Native CPU uses the best measured CPU
# configuration; all GPU targets use the fixed GPU configuration selected
# during tuning.
if [[ "$MODE" == "cpu" ]]; then
  VJP_TEAM_SIZE=8
  VJP_ITEMS_PER_LANE=4
  COMPACT_TEAM_SIZE=8
...
python - "$BUILD/variant.py" "dpcpp" "$MODE" "$TORCH_DEVICE" "$ARCH" \
